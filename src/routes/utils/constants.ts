export const SANS_UI_GITHUB_URL = 'https://github.com/sans-ui-org/sans-ui';
export const SANS_UI_GITHUB_CONTRIBUTING_URL =
	SANS_UI_GITHUB_URL + '/blob/development/CONTRIBUTING.md';
export const SANS_UI_FIGMA_URL =
	'https://www.figma.com/design/uXtJmwvaMFP8uYqb8ocWgm/S-UI-Designs?node-id=2337-3084&t=erTx2dAb5KbHXjcc-1';
export const SANS_DISCORD_URL =
	'https://discord.com/channels/1287433802494251019/1287433881720586242';
export const SANS_TWITTER_URL = 'https://x.com/sans_ui_org';
export const SANS_UI_LICENCE_URL =
	'https://github.com/sans-ui-org/sans-ui/blob/development/LICENSE';
('https://discord.com/channels/1287433802494251019/1287433881720586242');
export const SANS_DEVTO_URL = 'https://dev.to/sans-ui/';
export const SANS_MEDIUM_URL = 'https://medium.com/@three.s.ui.org';
export const SANS_HASHNODE_URL = 'https://hashnode.com/@SanSUI';
export const SANS_EMAIL_URL = 'mailto:three.s.ui.org@gmail.com';
export const SANS_UI_STORYBOOK_URL = 'https://sans-ui-storybook.vercel.app';

export const SANS_UI_GITHUB_REPO_URL = 'https://api.github.com/repos/sans-ui-org/sans-ui';
export const SANS_UI_GITHUB_API_URL = SANS_UI_GITHUB_REPO_URL + '/contributors?per_page=1';
